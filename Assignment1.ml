...
let rec between (n:int) (e:int):int list =
  if n>e then [] else
  let rec aux accum n e=
    if n = e then e::accum else
    aux (e::accum) n (e-1)
    in aux [] n e

let main =
  print_list(between 3 10)
(*
...
let zip_int (a: int list) (b: int list): (int*int) list=
  let rec aux accum a b=
    ...
    _,[] -> accum
    |
    ha::ta, hb::tb -> let x = (ha,hb) 
    in aux (x::accum) ta tb 
  in 
  let rec rev acc ls =
    match ls with
    []-> acc
...
let rec sort (ls: int list): int list = 
  let rec insert (i:int) (list: int list): int list =
    ...
    [] -> i::list
    |
    h::t -> if i < h then i::list else h::(insert i t)
    in 
  let rec aux ls =
    match ls with 
    [] -> []
    |head::tail -> insert head (aux tail)
    in aux ls 
